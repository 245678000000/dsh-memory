...
Requires Node `>=22.19` (same floor as DeepSeek Harness, for `node:sqlite`).
...
The bundle patch inserts plugin id `dsh-memory`. Restart the profile.
...
Override with `DSH_MEMORY_PATH` or:
...
Session 1 stores a global `pnpm` preference.  
Session 2 stores project Alpha `npm`. Asking “what should I use here?” in Alpha returns **npm**, with the global preference as background only.  
Session 3 supersedes VS Code with Cursor.  
Session 4 forgets Cursor. The next recall does not return it.
...
| Tool | Purpose |
| --- | --- |
| `memory_remember` | Explicit store (still runs policy) |
| `memory_search` | Explainable search |
| `memory_get` | Fetch one record |
| `memory_forget` | Forget by id / query / subject / scope |
| `memory_pin` / `memory_unpin` | Pin against GC |
| `memory_conflicts` | List disputes |
| `memory_resolve_conflict` | keep_a / keep_b / merge / remain_disputed / … |
| `memory_explain` | Why recalled / why not |
| `memory_list` | Filter by status, scope, kind |

`forget all` requires `confirmAll: true`.
